...
class ProfileExtractionService:
    ...
    async def start_profile_extraction(self, url: str, include_github: bool = False) -> str:
        ...
        asyncio.create_task(self._extract_profile_data(job_id, url))
        
        logger.info(f"Started profile extraction job {job_id} for URL: {url}")
        return job_id
    ...
    async def _extract_profile_data(self, job_id: str, url: str) -> None:
...
- experiences: Work history. (Use search to clarify dates/roles if ambiguous).
- education: Educational history.
- skills: Commercial and technical skills (Aggregate from all verified sources).
- projects: Notable public projects (Search for repo names, apps, or initiatives led by this person).
- achievements: Publicly verifiable awards (Search for press releases or announcements).
